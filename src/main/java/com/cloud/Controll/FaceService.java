...
//                return "5";
    ...
    @ResponseBody
    public  String  facecomp(@RequestParam(value = "imagefile") List<MultipartFile> files){
        Mat mat = null;
        String encodeurl = "http://"+url+"/locationEncode";
        ...
        for(MultipartFile file:files){
            try(CloudHttpServer httpServer = new CloudHttpServer()) {
                Mat borderMatpre = preProcess(file);
                //高斯滤波
                Mat borderMat = borderMatpre.clone();
                Imgproc.GaussianBlur(borderMatpre,borderMat,new Size(5,5),3,3);
                for(int i=0;i<borderMat.rows();i++)
                    for(int j=0;j<borderMat.cols();j++){
                        double c1 = borderMat.get(i,j)[0];
                        double c2 = borderMat.get(i,j)[1];
                        double c3 = borderMat.get(i,j)[2];
                        double[]c = {c1,c2,c3};
                        borderMat.put(i,j,c);
                    }
                normalize(borderMat,borderMat,0,255,CV_MINMAX);
                convertScaleAbs(borderMat,borderMat);
                MatOfByte matOfByte = new MatOfByte();
                Imgcodecs.imencode(".jpg",borderMat,matOfByte);
                byte[] result = Base64.encodeBase64(matOfByte.toArray());
                Map<String,Object> imageInfo = new HashMap<>();
                ...
                if(engineData.getEncodes().size()<1)
                    return "3";
                float[] boxes = engineData.getBoxs().get(0);
                Point p1 = new Point(boxes[0],boxes[1]);
                Point p2 = new Point(boxes[2],boxes[3]);
                Scalar scalar = new Scalar(255,0,0);
                Imgproc.rectangle(borderMat,p1,p2,scalar,4);
                MatOfByte matOfByte2 = new MatOfByte();
                Imgcodecs.imencode(".jpg",borderMat,matOfByte2);
                String result2 = Base64.encodeBase64String(matOfByte2.toArray());
                resultData.append(result2);
                resultData.append("########");
    ...
    @ResponseBody
    public String test2(){
        Mat mat = Imgcodecs.imread("D:/123.jpg");
        MatOfByte matOfByte = new MatOfByte();
        Imgcodecs.imencode(".jpg",mat,matOfByte);
        String result = Base64.encodeBase64String(matOfByte.toArray());
